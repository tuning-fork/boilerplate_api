...
describe User, type: :model do
  ...
  describe "#is_in_organization?" do
    ...
    it "returns true when user is in the provided organization" do
      organization = subject.organizations.first.id

      expect(subject.is_in_organization?(organization)).to be(true)
    end

    it "returns false when user is not in the provided organization" do
      organization = Organization.create!({ name: "New Org" })

      expect(subject.is_in_organization?(organization)).to be(false)
    end
  end
end
